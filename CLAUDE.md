...
- **Memory + automation**: `memory_tools.py` (REMEMBER/FORGET over `CallerMemoryStore.add_fact`/`remove_facts`), `persona_tool.py` (PERSONA — set/save/load the agent's demeanor for a call; active persona on `session.persona`, named profiles persisted in `data/personas.json` via `persona_store.py`, injected by `llm_engine._build_system_prompt`), `workflow_tool.py` (TRIGGER_WORKFLOW — fires webhooks named in `data/workflows.json` via `deliver_webhook`)
